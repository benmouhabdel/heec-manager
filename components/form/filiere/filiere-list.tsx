...
import { getFilieres } from "@/lib/actions/filiere";
...
export function FiliereList({ initialData }: FiliereListProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [isPending, startTransition] = useTransition();
  
  const [filieres, setFilieres] = useState(initialData?.filieres || []);
  ...
  const handleDepartementFilter = (value: string) => {
    setFilters(prev => ({ ...prev, departementId: value, page: 1 }));
    updateURL({ departementId: value, page: 1 });
  };
  ...
  const handleCreateNew = () => {
    router.push("/filieres/nouveau");
  };

  const handleEdit = (id: string) => {
    router.push(`/filieres/${id}/edit`);
  };

  const handleDelete = () => {
    loadFilieres(); // Reload after delete
  };
    ...
    <div className="space-y-6">
      ...
      {isPending && (
        <div className="flex items-center justify-center py-8">
          ...
          <span className="ml-2">Chargement des filières...</span>
        </div>
      )}
